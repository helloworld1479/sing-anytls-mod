...
func NewServerSession(conn net.Conn, onNewStream func(stream *Stream), _padding *atomic.TypedValue[*padding.PaddingFactory], logger logger.Logger) *Session {
	s := &Session{
		...
		padding:     _padding,
		logger:      logger,
		// 魔改 #2-1 (A2): shape the server->client (download) direction too.
		// Upstream leaves this false, so a stock AnyTLS server pads only the
		// client's uploads and sends the download direction's early packets
		// unshaped — the most exposed surface for DPI/ML flow fingerprinting.
		// Padding covers only the first `stop` packets of the session (per the
		// per-node scheme), so bulk throughput is unaffected.
		sendPadding: true,
	}
	s.die = make(chan struct{})
	s.streams = make(map[uint32]*Stream)
